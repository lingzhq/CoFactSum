...
def main():
    ...
    parser.add_argument('--topk_attn_model_path', default=None, type=str)
    ...
    parser.add_argument('--debias_ratio', default=None, type=float)
    parser.add_argument('--debias_ratio_2', default=None, type=float)
    ...
    topk_attn_model = MyPegasusForConditionalGeneration.from_pretrained(args.topk_attn_model_path) if args.topk_attn_model_path is not None else None
    model.resize_token_embeddings(len(tokenizer))

    if device == 'cuda': model.cuda()
    model.eval()
    if topk_attn_model is not None:
        if device == 'cuda': topk_attn_model.cuda()
        topk_attn_model.eval()
    print('Model loaded')
    ...
    with torch.no_grad():
        for batch_start in tqdm(range(0, len(sources), args.batch_size)):
            ...
            outputs = model.generate(**inputs,
                                     ...
                                     tokenizer=tokenizer,
                                     debias_ratio=args.debias_ratio,
                                     debias_ratio_2=args.debias_ratio_2,
                                     topk_attn_model=topk_attn_model,
                                     )
